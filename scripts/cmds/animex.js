...
commands.anigen = {
  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    try {
      ...
      const [prompt, ratio] = joinedArgs.split(/[-â€”]/).map(arg => arg.trim());
      ...
      console.error("Error:", error);
      message.reply("âŒ | Failed to generate image.");
    }
  }
};
...
commands.prodia = {
  ...
  onStart: async function ({ message, event, args, api }) {
    ...
    try {
      ...
      console.error("Error:", error);
      message.reply("âŒ | An error occurred.");
    }
  }
};
commands.replicate = {
...
commands.xl = {
  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    try {
      ...
      if (!prompt) {
        message.reply("âŒ | Please provide a prompt.");
        return;
      }
  ...
  onStart: async function ({ message, event, args, api }) {
    ...
    try {
      ...
      if (args.length > 0) {
        ...
        return message.reply("âŒ | Please provide a prompt.");
      }
